...
S_air = 4*pi*r_air*r_air  + 2*pi*r_air*cylinder  #[m2]
...
for i in range(0, len(material_drone)):
    for j in range(0, len(amount_rot)):
        V_frame = amount_rot[j]*r_arm*0.5*2*pi*t_arm*w_arm + A_extra*0.004  #[m3]
...
#S_w = np.arange(5.38196 ,16.1459, 0.5) #Wingarea [ft2]
#W_fw = 0.564/0.453592                                        #weight fuel wing [lb]
#A = np.arange(5,10,0.05)
#sweep = np.arange(0.523599, 0.872665,0.1)                        #Aspect ratio [-]
#taper = np.arange(0.2, 0.5, 0.01)                            #taper [-]
#q = 11.513                                                   #dynamic pressure at cruise [lb/ft2]
