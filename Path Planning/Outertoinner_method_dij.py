...
fix = True  #True means pre-routed as already determined when set to False
...
#------------------------------------------------------------------------------------------------#
#Dijkstra taken from https://dev.to/mxl/dijkstras-algorithm-in-python-algorithms-for-beginners-dkc    ---  Referenced in report
...
def points(r):
    '''Returns number of points that may be covered depending on distance [time dependent]'''
    return int((ttot-2*r/Vdr + space/Vdr)/(thov+space/Vdr))

def pointsE(r):
    '''Returns number of points that may be covered depending on distance [energy dependent]'''
    return int((603.8399999999999 -1251.7675454800244*2*(r/Vdr)*(1/3600) + 5.703547149411245 )/(1.4167467541112169 + 5.703547149411245 ))
