...
radm = 5000
space = 550
sfac = 0.2  #size of circles and linewidth
...
#Runway coordinates
rwy36Lcoords = [(-3482,2307), (-2882,12060), (-2670,12000), (-3270,2266)]
rwy24coords = [(1424,-245),(1560,-460),(-4460,-3850),(-5000,-6000),(-6000,-4409)]
    ...
    for yyl in yl:
        if rwy36Lpoly.contains(Point(xxl,yyl)) == False and rwy24poly.contains(Point(xxl,yyl)) == False:
            if yyl ** 2 + xxl ** 2 <= radm ** 2:
                ...
                coordsindex.append(count)
                coordsindexstr.append(str(count))
                count += 1
...
#POINT CHECKER
checkval = int(138)  #12 #74 #233
#draw lines for movement of chess pieces
for i in cnext[checkval]:
    plt.plot([i[0],coordsorig[checkval][0]],[i[1],coordsorig[checkval][1]],linewidth=0.37,color=(0,0,0))
#draw actual points
plt.scatter([item[0] for item in cnext[checkval]],[item[1] for item in cnext[checkval]],color=(0,0.7,0.1))
plt.scatter([coordsorig[checkval][0]],[coordsorig[checkval][1]],color='r')
...
#SD
#plt.savefig('2_PP_SegmentedXXXX.png',dpi=400)
