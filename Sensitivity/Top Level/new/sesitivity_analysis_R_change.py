...
#rankings:
f = open("sensitivity_ranks.txt",'w')
...
#only risks will stay like this!!
    ...
    f.writelines("\n"+str(rnk)+"\n")
    f.writelines(str(names[np.argsort(grades)[2]]+': '+str(round(grades[np.argsort(grades)[2]],3))+'  '+names[np.argsort(grades)[1]]+': '+str(round(grades[np.argsort(grades)[1]],3))+'  '+names[np.argsort(grades)[0]]+': '+str(round(grades[np.argsort(grades)[0]],3))+'\n'))
    #f.writelines(str(names[np.argsort(grades)[2]]))  uncomment for just first

print('\ntotal combinations: '+str(len(ranks)))
